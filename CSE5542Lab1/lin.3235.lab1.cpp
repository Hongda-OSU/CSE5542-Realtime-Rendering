...
int main(int argc, char **argv)
{
    ...
    glutInit(&argc, argv);
    glutInitDisplayMode(GLUT_DOUBLE | GLUT_RGBA); // double-buffering and RGB color
                                                 // mode.
    glutInitWindowSize(500, 500);
    glutInitWindowPosition(500, 300);  // Graphics window position
    glutCreateWindow("Aha, my first OpenGL programming! "); // Window title is "Rectangle"
   ...
   * Once entering the event loop, the execution control always returns to
    ...
    */
    ...
    glutReshapeFunc(reshape); // Register our reshape() function as the reshape
                              // call-back function. It manages resizing events.
...
void display(void)
{
    ...
    glBegin(GL_POLYGON);
    glVertex2f(25, 300 + f);
    glVertex2f(275, 300 + f);
    glVertex2f(275, 400 + f);
    glVertex2f(25, 400 + f);
    glEnd();
...
void idle(void)
{
    f += 0.01f; // smaller number gives a slower but smoother animation
